...
export class NodeID3 {
  ...
  public write(
    tags: Tags,
    filebuffer: string | Buffer,
    fn?: (err: NodeJS.ErrnoException | Error | null, buffer?: Buffer) => void,
  ) {
  ...
  public read(
    filebuffer: string | Buffer,
    options?: ((err: NodeJS.ErrnoException | null, data: any) => void) | object,
    fn?: (err: NodeJS.ErrnoException | null, data: any) => void,
  ): Tags | void {
    ...
    if (!fn || typeof fn !== 'function') {
      ...
      Object.keys(rawTags).map((tag) => {
        if (
          ...
          SFrames[SRawToNameMap[tag]].updateCompareKey &&
          currentTags[tag] &&
          rawTags[tag]
        ) {
  ...
  protected getFramesFromID3Body(
    ID3FrameBody: Buffer,
    ID3Version: number,
    identifierSize: number,
    textframeHeaderSize: number,
  ): DescribedFrame[] {
   ...
   **  }
   **/
  protected createUserDefinedText(
    userDefinedText: UserDefinedText | UserDefinedText[],
    recursiveBuffer?: Buffer,
  ): Buffer {
